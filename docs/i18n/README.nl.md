...
- 🔌 **Poorten** — elke TCP-poort waarop wordt geluisterd, naast het proces dat hem bezet: dubbelklik op een rij om `localhost:3000` in de browser te openen, rechtsklik om dat proces met zijn onderliggende processen te beëindigen. Handig om de vergeten dev-server terug te vinden; systeemservices blijven standaard verborgen.

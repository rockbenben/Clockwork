...
- 🔌 **Ports** — chaque port TCP en écoute, à côté du processus qui l'occupe : double-cliquez sur une ligne pour ouvrir `localhost:3000` dans le navigateur, clic droit pour terminer ce processus et ses processus enfants. Pratique pour retrouver le serveur de dev oublié ; les services système restent masqués par défaut.

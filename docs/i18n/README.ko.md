...
> **전역 단축키** — 그룹 편집기에서 단축키 상자를 클릭하고 단축키(예: `Ctrl+Alt+F`)를 누르면 메뉴 없이 어디서나 해당 그룹을 실행합니다. Esc로 취소, Delete로 지웁니다. 비활성화된 그룹은 조합을 해제합니다; 시스템 예약 조합(Alt+F4, Ctrl+Shift+Esc…)과 다른 그룹이나 긴급 정지 단축키가 이미 사용 중인 조합은 알림과 함께 거부됩니다.

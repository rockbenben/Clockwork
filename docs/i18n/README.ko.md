...
동작을 재사용 가능한 그룹으로 묶습니다. **추가 ▾**는 **기본 제공 템플릿**(집중 / 회의 / 마무리 / 취침 / 자리 비움 / 스크린샷)에서 시작합니다 — 프로세스 이름을 조정하고 저장하기만 하면 됩니다. 그룹은 **동작을 정의할 뿐**입니다; 네 가지 방법으로 트리거합니다: 트레이에서(**실행: <그룹>**), **전역 단축키**로, 시작 목록의 **액션 그룹 단계**로(부팅 시), 또는 알림에서(**"예" / 무음 그룹**). 한 그룹은 한 번에 하나만 실행됩니다; **메시지** 단계는 확인 관문 역할을 할 수 있습니다(**아니요**로 답하면 나머지가 중단됨).
...
**설정 내보내기 / 설정 가져오기** — 전체 설정을 다른 PC로 옮기거나 백업으로 보관하세요. 내보내기는 `clockwork.settings.json` 사본을 원하는 위치에 저장합니다; 가져오기는 **전부**(시작 목록 / 알림 / 액션 그룹 / 설정)를 덮어쓰므로 먼저 확인을 받고, 현재 설정을 `clockwork.settings.json.bak`으로 백업한 뒤, 적용을 위해 앱을 다시 시작합니다.

## 팁

- **행을 더블클릭해 편집**하세요. 경로 / 프로세스 / 단축키 / 날짜를 채울 때 손으로 입력할 필요가 없습니다: **찾아보기…**, **선택…**(검색 가능한 프로세스 선택기), **캡처**, **날짜 선택**이 있습니다.
- **복제**(알림 / 액션 그룹 탭)는 선택한 행을 바로 아래에 복제합니다 — 비슷한 항목을 처음부터 다시 만드는 것보다 빠릅니다; 복제된 그룹의 이름은 "… (사본)"이 됩니다.

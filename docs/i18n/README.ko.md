...
동작을 재사용 가능한 그룹으로 묶습니다. **추가 ▾**는 **기본 제공 템플릿**(집중 / 회의 / 마무리 / 취침 / 자리 비움 / 스크린샷)에서 시작합니다 — 프로세스 이름을 조정하고 저장하기만 하면 됩니다. 그룹은 **동작을 정의할 뿐**입니다; 네 가지 방법으로 트리거합니다: 트레이에서(**실행: <그룹>**), **전역 단축키**로, 시작 목록의 **액션 그룹 단계**로(부팅 시), 또는 예약 작업에서(**"예" / 무음 그룹**). 한 그룹은 한 번에 하나만 실행됩니다; **메시지** 단계는 확인 관문 역할을 할 수 있습니다(**아니요**로 답하면 나머지가 중단됨).

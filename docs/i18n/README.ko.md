...
- **액션 그룹** — 정의된 액션 그룹을 실행; 그 반복 횟수는 *이 참조*가 몇 번 발동하는지를 뜻합니다(그룹 자체도 내부적으로 반복할 수 있습니다 — 아래 **액션 그룹** 참고).
...
> **언제든 중지** — 창 탭 바 오른쪽 끝의 **중지 버튼**(실행 중일 때만 표시), 트레이 → **실행 중인 동작 중지**, 또는 전역 **긴급 정지 단축키**(설정 탭에서 지정; 기본값 `Ctrl+Alt+Q`). 실행 중인 것은 현재 동작이 끝난 뒤 중지됩니다; 긴 대기(시작 지연, 창 대기)는 즉시 중단됩니다.

### 예약 작업

**시간**(또는 **로그인 시**로 전환), **반복**(요일 / N일마다 / 매월 / **지정한 날짜에 한 번만**)을 설정하고 **동작**을 하나 고릅니다: **알림 띄우기** 또는 **액션 그룹 무음 실행**. 선택한 동작의 입력란만 화면에 남으므로, 아무 효과도 없는 칸을 채우는 일이 없습니다.

**"예" 동작**(프로그램 실행 / 파일 열기 / URL / 액션 그룹 실행)이 있는 알림은 **다시 알림** 버튼이 있는 **예 / 아니요** 대화 상자를 띄웁니다(기본 10분, ▾ 메뉴로 5–60분); 나머지는 구석에 **알림 카드**로 미끄러져 들어옵니다(설정한 초 후 자동으로 닫힘, **0 = 직접 닫을 때까지 유지**). 텍스트는 소리 내어 읽어 줄 수도 있습니다.

**간격 실행**은 하나의 작업을 하루 종일의 일정으로 바꿉니다: *N분마다 HH:mm까지*(비우면 그날 끝까지). 확인하는 순간 멈추는 **미확인 시 재촉**과 달리, 간격 실행은 답한 뒤에도 계속됩니다 — 무음 그룹을 폴링으로 쓸 수 있는 이유가 바로 이것입니다. 간격은 자정을 넘지 않으며, 다음 날은 작업 자신의 시각에서 새로 시작합니다. 진행 상황은 저장되므로 정오에 앱을 다시 시작해도 그날 남은 회차를 잃지 않습니다.

**한 번만**은 지정한 날짜에 발동한 뒤 **스스로 체크를 해제**하고 목록에는 그대로 남습니다 — 날짜를 바꿔 다시 체크하면 재사용할 수 있습니다. 재촉이나 다시 알림이 끝날 때까지 기다린 뒤 꺼지므로 전달을 중간에 끊지 않습니다.

응답 없는 대화 상자는 길을 막지도, 사라져 버리지도 않습니다. 최대 1분 뒤 자동으로 "**10분 다시 알림**"으로 전환되어 나중에 다시 나타납니다. 이 상태는 손으로 누른 다시 알림과 똑같이 디스크에 저장되어 재시작해도 유지되며, "**놓치면 따라잡기**"를 켜 두면 자정을 넘겨 잠들어도 다음 날 한 번 다시 발화합니다. 같은 알림이 반복 발화하면 카드 한 장에 합쳐지고(**×N** 표시), 닫았거나 자동으로 사라진 카드는 트레이의 "**최근 알림**" 메뉴에서 다시 볼 수 있습니다.
...
**자동으로 시작되는 모든 항목**을 나열합니다(레지스트리 Run 키, 시작 폴더, 예약 작업). **사용** 체크를 해제하면 항목을 끕니다 — **삭제가 아니라 비활성화; 다시 체크하면 복원**(즉시 적용). **관리자 필요**로 표시된 항목은 권한을 상승하여 다시 실행하도록 안내합니다. 시스템 / 정책 / 일회성 항목(그룹 정책 Run, RunOnce, Winlogon, Active Setup)은 건드릴 수 없으며 **기본적으로 숨겨져** 있습니다 — **시스템 / 읽기 전용 항목 표시**에 체크하면 볼 수 있습니다(회색으로 표시). 행을 **우클릭**하면 **시작 목록으로 가져오기**(항목을 Clockwork에 넘김; 레지스트리 Run 키와 시작 폴더 항목만) 또는 **시스템에서 삭제**(항목을 완전히 제거 — 먼저 확인하며 되돌릴 수 없음; 체크 해제가 되돌릴 수 있는 선택지)를 쓸 수 있습니다. 상단의 **필터**는 이름 / 명령으로 검색합니다; 잘린 명령 위에 마우스를 올리면 전체를 읽을 수 있습니다.

### 액션 그룹

동작을 재사용 가능한 그룹으로 묶습니다. **추가 ▾**는 **기본 제공 템플릿**(집중 / 회의 / 마무리 / 취침 / 자리 비움 / 스크린샷 / 오래 앉기 알림)에서 시작합니다 — 프로세스 이름을 조정하고 저장하기만 하면 됩니다. 그룹은 **동작을 정의할 뿐**입니다; 네 가지 방법으로 트리거합니다: 트레이에서(**실행: <그룹>**), **전역 단축키**로, 시작 목록의 **액션 그룹 단계**로(부팅 시), 또는 예약 작업에서(**"예" / 무음 그룹**). 한 그룹은 한 번에 하나만 실행됩니다.

**반복.** 그룹은 **전체를 반복**할 수 있습니다(반복 횟수 + 회차 간 간격, 그룹 편집기에서 설정). 순서의 *일부만* 반복하려면 그 단계들을 별도 그룹으로 만들고 **액션 그룹** 단계로 참조하면서 반복 횟수를 지정하세요 — 그룹은 그룹을 참조할 수 있고, 저장할 때 순환 참조는 사슬을 그대로 보여 주며 거부됩니다(`A → B → A`). 세 가지 반복 값은 곱해집니다: 단계별 × 참조별 × 그룹 전체. 한 번의 실행에는 **5000단계** 퓨즈가 있어, 지나치게 큰 조합은 끝없이 돌지 않고 멈춥니다.

**메시지** 단계는 확인 관문입니다: **아니요**로 답하면 그 그룹의 남은 단계*와 남은 회차*가 중단되고, 이를 참조한 그룹으로까지 바깥쪽으로 전파됩니다 — ×N으로 반복되는 하위 그룹 안에서도 한 번 거절하면 끝입니다.

> **전역 단축키** — 그룹 편집기에서 단축키 상자를 클릭하고 단축키(예: `Ctrl+Alt+F`)를 누르면 메뉴 없이 어디서나 해당 그룹을 실행합니다. Esc로 취소, Delete로 지웁니다. 비활성화된 그룹은 조합을 해제합니다; 시스템 예약 조합(Alt+F4, Ctrl+Shift+Esc…)과 다른 그룹이나 긴급 정지 단축키가 이미 사용 중인 조합은 알림과 함께 거부됩니다. **단축키는 토글입니다:** 그룹이 실행 중일 때 다시 누르면 그 실행을 취소합니다 —— 다른 그룹과 시작 목록은 계속 실행됩니다(전부 멈추는 것은 긴급 정지 단축키입니다). 이렇게 취소할 수 있는 것은 단축키를 지정한 그룹뿐입니다.
...
**설정 내보내기 / 설정 가져오기** — 전체 설정을 다른 PC로 옮기거나 백업으로 보관하세요. 내보내기는 `clockwork.settings.json` 사본을 원하는 위치에 저장합니다; 가져오기는 **전부**(시작 목록 / 예약 작업 / 액션 그룹 / 설정)를 덮어쓰므로 먼저 확인을 받고, 현재 설정을 `clockwork.settings.json.bak`으로 백업한 뒤, 적용을 위해 앱을 다시 시작합니다.

...
2. **`Clockwork.exe`**를 더블클릭해 설정 창을 엽니다.
   - **첫 실행** 시 시작 목록과 알림에 몇 가지 **샘플**이 로드되므로 자신에게 맞게 고칠 수 있습니다 — 모두 처음에는 체크가 꺼져 있어, 직접 체크하기 전에는 아무것도 실행되지 않습니다. 설정은 exe 옆의 `clockwork.settings.json`에 저장됩니다 — 로컬 전용이며 절대 커밋되지 않습니다.
...
동작을 재사용 가능한 그룹으로 묶습니다. **추가 ▾**는 **기본 제공 템플릿**(집중 / 회의 / 마무리 / 취침 / 자리 비움 / 스크린샷)에서 시작합니다 — 프로세스 이름을 조정하고 저장하기만 하면 됩니다. 그룹은 **동작을 정의할 뿐**입니다; 네 가지 방법으로 트리거합니다: 트레이에서(**실행: <그룹>**), **전역 단축키**로, 시작 목록의 **액션 그룹 단계**로(부팅 시), 또는 예약 작업에서(**"예" / 무음 그룹**). 한 그룹은 한 번에 하나만 실행됩니다.

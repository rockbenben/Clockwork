...
| **Send keys** | e.g. Win+D, Alt+K, Ctrl+Enter, F5 (supports Enter / Tab / Esc / Del / arrows…; **Capture** records a shortcut by pressing it). **Wheel**: double-click the box and type `WheelUp` / `WheelDown` for one notch of scroll — pair it with this step's **repeat count** for N notches and the post-step delay for the rhythm. Modifiers work as usual, so `Ctrl+WheelDown` zooms. Which window receives it depends on Windows' “Scroll inactive windows when I hover over them” setting (on by default on Win10/11 = the window under the pointer; off = the focused window), so pair it with **Window action → bring to front** when it matters. Wheel can be sent but not bound as a global hotkey — the OS hotkey registration cannot express it. |

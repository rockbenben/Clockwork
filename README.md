...
- 🔌 **Ports** — every TCP port being listened on, next to the process holding it: double-click a row to open `localhost:3000` in your browser, right-click to free the port, ending every process holding it along with their children. Handy for tracking down the dev server you forgot to stop; system services stay hidden by default.

...
进阶选项：**自动关闭**（N 秒没点击就自己关，适合"看到就行"的提醒）、**重复催促**（没确认就每 N 分钟再弹、直到某时刻，适合催睡觉这类）、**到点后延迟 + 随机延迟**、**宽限**（到点时恰好关机 / 睡眠，N 分钟内补弹）、「每 N 天」的**起算日**（可点「选日期」）。

### 系统启动项
列出电脑里**所有开机自启**的项目（来自注册表 Run 键、启动文件夹、计划任务）。取消某项的「启用」勾选即禁用——**只是禁用、不会删除，随时再勾上就恢复**（勾选**即时生效**）。带「需管理员」的项要管理员权限：勾选时会提示以管理员身份重开再操作。少数**系统 / 策略项**（组策略 Run、RunOnce 一次性项、Winlogon、Active Setup）无法用常规开关关闭，列表里以**置灰的「只读」**标出、不可勾选，仅供查看。「刷新」重新扫描；「纳入启动清单」把某项交给本工具接管（自动停用原项 + 加进你的清单）。项目多时，顶部**过滤框**可按名称 / 命令快速筛。
...
- **请用正常方式启动本程序**（双击 `开机助手-双击运行.bat`、托盘、或计划任务）。若用某些**沙箱 / 受限权限的启动器**（如 Lucy）打开，Windows 会限制运行时编译，**发送按键 / 窗口动作 / 已运行则激活 / 发送文本到指定进程 / 音量** 等原生功能会不可用（会给出清楚提示、并不影响单纯「启动程序」）。

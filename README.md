...
> **随时叫停**：托盘右键 →「**停止正在运行的动作**」，或按全局**急停键**（在主窗底部设置，清空 = 禁用；全新安装预置 `Ctrl+Alt+F12`，升级用户默认不占用系统热键、需自行设一个）。正在运行的启动清单 / 动作组 / 单步运行会在当前动作做完后立刻停下（循环设多了 / 延时设长了不用干等）；开机延迟、等窗口出现等长等待也会被立即打断。启动日志会如实记录「已手动停止」和停在哪一步。若设的热键被别的程序占用而注册失败，会弹托盘气泡提醒（此时用托盘菜单的「停止」兜底）。
...
- **请用正常方式启动本程序**（双击 `Clockwork.bat`、托盘、或计划任务）。若用某些**沙箱 / 受限权限的启动器**（如 Lucy）打开，Windows 会限制运行时编译，**发送按键 / 窗口动作 / 已运行则激活 / 发送文本到指定进程 / 音量** 等原生功能会不可用（会给出清楚提示、并不影响单纯「启动程序」）。
...
- 逻辑层 `lib/Clockwork.{Core,Actions,SystemStartup,Win32}.ps1` 是纯函数、可测；界面层 `lib/Clockwork.Wpf*.ps1` 用 WPF。
- 跑测试：
  ```powershell

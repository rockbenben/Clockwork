...
2. Double-click **`Clockwork.exe`** to open the settings window.
   - On **first run** it loads a few **samples** in the startup list and the scheduled tasks so you can adapt them to your own — all of them start unticked, so nothing runs until you tick it. The Action groups tab also starts with two ready-to-run groups (Stepping away / Wrap-up) — those *are* ticked, because a group never fires on its own; it only runs when you trigger it. Your settings live in `clockwork.settings.json` next to the exe — local only, never committed.
...
Bundle actions into a reusable group. **Add ▾** starts one from a **built-in template** (Focus / Meeting / Wrap-up / Bedtime / Stepping away / Screenshot / Sitting too long) — tweak the process names and save. A group **only defines actions**; trigger it four ways: from the tray (**Run: <group>**), a **global hotkey**, an **action-group step** in the startup list (at boot), or a scheduled task (**On-Yes / silent group**). A group runs only one copy at a time.

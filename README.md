...
- 🔌 **Ports** — every TCP port being listened on, next to the process holding it: double-click a row to open `localhost:3000` in your browser, right-click to end that process along with its children. Handy for tracking down the dev server you forgot to stop; system services stay hidden by default.
